...
const load_model_callback = (data) => {
    const {status} = data
    if (status === 'progress') {
...
const sendDownloadingMessage = async (file, progress, loaded, total) => {
    self.postMessage({
...
const createResultMessage(results, isDone, completedUntilTimestamp) {
    self.postMessage({
...
const createPartialResultMessage(result) {
    self.postMessage({
